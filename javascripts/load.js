  ...
  //  alert("entred");
    ...
    if(classname1 == 'open-with'){
      ...
      $("#rname").click(function(e){
        ...
        if(newname == '.' || newname == '..' || newname == '' || (newname.indexOf('/') >= 0) || (newname.indexOf("\\") >= 0)){
          $('.rename-modal-container p').text("Please Enter a valid name");
          $('.rename-modal-container p').css({'color':'red'});
          return;
        }
        if(newname.length > 20){
          $('.rename-modal-container p').text("Name Limit 20 Characters");
          $('.rename-modal-container p').css({'color':'red'});
          return;
        }
        src = oldname;
        ...
        $.ajax({
          ...
          success:function(result){
            if(result != 1){
              ...
              $('.rename-modal-container p').text("Error while renaming");
              $('.rename-modal-container p').css({'color':'red'});
            }
            $( ".modal-background-filter" ).remove();
            $( ".open-modal" ).remove();
...
function flipdel(file){
  ...
  $.ajax({
    ...
    success:function(result){
      if(result!=1)
      {
            $(".body").append('<div class="modal-background-filter"></div><div class="open-modal error-modal-container" ><p>Error Restoring/Deleting File</p><div class="okay button-done" id="okay" >OK</div><div class="close-button close"><i class="close-button ion-close"></i></div></div>');
      }
       //alert(result); //TODO Change Later
    }
  });
}
fd = flipdel;
